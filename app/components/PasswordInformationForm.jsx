...
export default function PasswordInformation(){
    ...
    <form action={dispatch} className='xl:mt-10 w-full h-fit max-w-2xl flex flex-col gap-7 rounded-xl border border-gray-200 dark:border-gray-700 bg-white/20 dark:bg-black/20  shadow-[inset_10px_-50px_94px_0_rgb(199,199,199,0.2)] backdrop-blur p-5'>
        ...
        <div className="flex flex-col space-y-1">
                ...
                <input required placeholder='********' name="current_password" type="password" id="current_password" className="text-gray-500 dark:text-white dark:bg-white/10 outline-0 focus:border-black dark:focus:border-white focus:outline-none focus:ring-0 border border-gray-200 dark:border-gray-700 font-semibold  p-2 rounded-md w-[280px] disabled:bg-white" />
            </div> 

            <div className="flex flex-col space-y-1">
                <label htmlFor="new_password" className="text-sm font-semibold">New password</label>
                <input required placeholder='********' name="new_password" type="password" id="new_password" className="text-gray-500 dark:text-white dark:bg-white/10 outline-0 focus:border-black dark:focus:border-white focus:outline-none focus:ring-0 border border-gray-200 dark:border-gray-700 font-semibold  p-2 rounded-md w-[280px] disabled:bg-white" />
            </div> 
        </div>
